...
numInjections = int(sys.argv[4]) # If it is more than 10, change buffer_size in test_ds

totsdc = 0.0

start = time.time()
for i in range(numFaults):
    train_labels1 = copy.deepcopy(train_labels)
    train_images1 = copy.deepcopy(train_images)
    model.load_weights('h5/alexnet-trained.h5')
    ...
    model.load_weights('h5/alexnet-untrained.h5')
    train_images_,train_labels_ = tfi.inject(x_test=train_images1,y_test=train_labels1, confFile=conf)
    train_ds_ = tf.data.Dataset.from_tensor_slices((train_images_, train_labels_))
    train_ds_ = (train_ds_
            .map(process_images)
            .shuffle(buffer_size=train_ds_size)
            .batch(batch_size=32, drop_remainder=True))
